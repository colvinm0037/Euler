...
namespace Euler
{
    public class Euler_011 : IEulerProblem
    {
        ...
        private string Main()
        {
            ...
            // Find the max from each row
            for (int i = 0; i < 20; i++)
            {
                long sum = HandleDigits(grid[i]);
                if (sum > max)
                    max = sum;
            }
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                long sum = HandleDigits(column);
                if (sum > max)
                    max = sum;

            }

            //Console.WriteLine("Diagonals 1");
            for (int i = 1; i < 21; i++)
            {
                int[] column = new int[i];
                for (int k = 0; k < i; k++)
                {
                    column[k] = grid[k][i - k - 1];
                }
            ...
            //Console.WriteLine("Diagonals 2");
            for (int i = 19; i > 0; i--)
            {
                int[] column = new int[20 - i];
                int count = 0;
                for (int k = 19; k >= i; k--)
                {
                    column[19 - k] = grid[k][i + count];
                    count++;
                }
            ...
            //Console.WriteLine("Diagonals 3");
            for (int i = 1; i < 21; i++)
            {
                int[] column = new int[i];
                for (int k = 0; k < i; k++)
                {
                    column[k] = grid[k][20 - i + k];
                }
            ...
            //Console.WriteLine("Diagonals 4");
            for (int i = 1; i < 21; i++)
            {
                int[] column = new int[i];
                for (int k = 0; k < i; k++)
                {
                    column[k] = grid[20 - i + k][k];
                }
        ...
        static long HandleDigits(int[] numbers)
        {
            long max = 0;
